...
@Component
public class GetLatestHighlightsTask {
    ...
    public GetLatestHighlightsTask(HighlightClient highlightClient, BroadcasterRepository broadcastersRepository, HighlightTaskUtil highlightTaskUtil) {
    ...
    protected void getLatestHighlights() {
        logger.info("Get latest highlights");
        ...
        logger.info("Finished getting latest highlights");
    }

    private void getAllHighlightsBroadcaster(Broadcaster broadcaster) {
        PagedResponse<Video> highlightResponse = highlightClient.getLatestHighlightsUncached(broadcaster.getId());
